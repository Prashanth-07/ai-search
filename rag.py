...
# def convert_tools_to_documents(tool_data):
#     """
#     Accepts a list of dictionaries representing tool data and converts them into Document objects.
#     Each dictionary should have keys: 'name', 'description', and 'api_details'.
#     """
#     documents = []
#     for tool in tool_data:
...
# def format_docs(docs):
#     """
#     Formats a list of Document objects into a single string context.
#     """
#     return "\n\n".join([doc.page_content for doc in docs])

# def setup_vector_store(documents):
#     """
#     Creates a FAISS vector store from the list of documents using an Ollama embedding model.
#     """
#     # Initialize the embedding model – adjust model and base_url as needed.
#     embeddings = OllamaEmbeddings(model='nomic-embed-text', base_url="http://localhost:11434")
#     # Compute a sample vector to determine dimensionality.
#     sample_vector = embeddings.embed_query("sample text")
#     index = faiss.IndexFlatL2(len(sample_vector))
...
# def create_rag_chain(retriever):
#     """
#     Sets up the RAG chain using a prompt template and a ChatOllama model.
#     The prompt instructs the model to select the best matching AI tool based on the retrieved context.
#     """
#     prompt = """
#     You are an assistant that selects the best matching AI tool based on user requirements.
#     Use the following tool information to answer the query.
...
#     Answer in bullet points:
#     """
#     # Initialize the language model – adjust model and base_url as needed.
#     model = ChatOllama(model="deepseek-r1:1.5b", base_url="http://localhost:11434")
#     prompt_template = ChatPromptTemplate.from_template(prompt)
    
#     # Chain the retriever with formatting and language model to form the RAG pipeline.
#     rag_chain = (
#         {"context": retriever | format_docs, "question": RunnablePassthrough()}
#         | prompt_template
#         | model
...
# def run_rag_system(tool_data, user_query):
#     """
#     Accepts tool data (a list of dictionaries) and a user query, then runs the RAG system to generate an answer.
#     """
#     documents = convert_tools_to_documents(tool_data)
#     if not documents:
#         return "No tool data provided."
    
#     vector_store = setup_vector_store(documents)
#     # Create a retriever from the vector store (using MMR with top 3 matches)
#     retriever = vector_store.as_retriever(search_type="mmr", search_kwargs={'k': 3})
#     rag_chain = create_rag_chain(retriever)
...
# def main():
#     # Sample tool data; this simulates the documents you'd normally retrieve from MongoDB.
#     tool_data = [
#         {
#             "name": "VintageImageGen",
#             "description": "Generates vintage-style images using AI algorithms.",
...
#     # Accept a user query (e.g., "I want an image generation tool that generates vintage images.")
#     user_query = input("Enter your tool query: ")
#     print("\nRetrieving and generating answer...\n")
#     answer = run_rag_system(tool_data, user_query)
#     print("\n\nFinal Answer:\n", answer)
